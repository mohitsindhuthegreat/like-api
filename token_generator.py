...
class TokenGenerator:
    ...
    def get_token(self, password: str, uid: str) -> Optional[Dict]:
        ...
        try:
            ...
            if res.status_code != 200:
                logger.warning(f"Failed to get token for UID {uid}: HTTP {res.status_code}")
                return None
    ...
    def generate_jwt_token(self, uid: str, password: str) -> Optional[Dict]:
        """Generate JWT token using the full process"""
        try:
    ...
    def generate_tokens_for_region(self, account_file: str, output_file: str, region_name: str) -> int:
        ...
        for i, account in enumerate(accounts, 1):
            try:
                ...
                token_result = self.generate_jwt_token(uid, password)
                if token_result:
