...
class TokenGenerationCooldown(db.Model):
    __tablename__ = 'token_generation_cooldowns'
    
    id = db.Column(db.Integer, primary_key=True)
    uid = db.Column(db.String(20), nullable=False, index=True, unique=True)
    server_name = db.Column(db.String(10), nullable=False, index=True)
    last_generated = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    
    # Create composite index for faster queries
    __table_args__ = (
        db.Index('idx_uid_cooldown', 'uid', 'last_generated'),
    )
    ...
    def __repr__(self):
        return f'<TokenGenerationCooldown {self.uid} - {self.server_name}>'
